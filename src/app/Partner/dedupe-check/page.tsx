...
export default function PartnerDashboard() {
    ...
    const postToInfiSchemaInBatches = async (rows: ExcelRow[], headers: string[]) => {
        const INFI_SCHEMA_BATCH_SIZE = 50; // Adjust this value if you still get the error
        let success = true;

        for (let i = 0; i < rows.length; i += INFI_SCHEMA_BATCH_SIZE) {
            const batch = rows.slice(i, i + INFI_SCHEMA_BATCH_SIZE);

            if (batch.length > 0) {
                const dataPayload = batch.map(row => {
                ...
                while (attempt < MAX_RETRIES && !batchSuccess) {
                    try {
                        ...
                        console.warn(`Post to infiSchema failed (batch ${i/INFI_SCHEMA_BATCH_SIZE + 1}), attempt ${attempt + 1}:`, err);
                        attempt++;
                        if (attempt < MAX_RETRIES) {
                            await sleep(RETRY_DELAY * attempt);
                        } else {
                            console.error(`Post to infiSchema failed after ${MAX_RETRIES} attempts.`);
                            success = false; // Mark overall process as failed
                        }
                    }
                }
                if (!batchSuccess) {
                    return false; // Exit early if a batch fails
                }
            }
        }
        return success;
    ...
    const processBatch = useCallback(
        async (
            ...
            if (success) {
                ...
                const batchNotDuplicates: ExcelRow[] = [];
                ...
                rows.forEach((row) => {
                    ...
                    if (status === 'Duplicate') {
                        ...
                        tempNonDup.push(row);
                        batchNotDuplicates.push(row);
                    }
                });

                // Post 'Not Duplicate' records to the new API in smaller batches
                if (batchNotDuplicates.length > 0) {
                    const postSuccess = await postToInfiSchemaInBatches(batchNotDuplicates, headers);
                    if (!postSuccess) {
                        // Handle the case where the post failed after all retries
                        console.error('Failed to post some "Not Duplicate" records to infiSchema.');
                    }
                }
            }
