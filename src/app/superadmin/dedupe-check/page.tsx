...
type ApiResponseItem = {
  phone: string | number;
  status: 'Duplicate' | 'Not Duplicate';
};
...
export default function PartnerDashboard() {
  ...
  const processBatch = async (
    ...
    while (attempt < MAX_RETRIES && !success) {
      try {
        ...
        resultData = json.phoneData || []; // ✅ Updated for new API
        success = true;
      } catch (err) {
    ...
    if (success) {
      ...
      rows.forEach((row) => {
        const phone = String(row[phoneIndex]).replace(/\D/g, '').slice(-10);
  ...
  const processData = async (allRows: ExcelRow[], phoneIndex: number) => {
    setProcessing(true);
    setUploadMessage('Checking for duplicates...');
    setProgress(0);
    ...
    if (allRows.length === 0) {
      setUploadMessage('✅ No data rows found.');
      setProcessing(false);
      return;
    }
    ...
    for (let i = 0; i < allRows.length; i += BATCH_SIZE) {
      ...
      await processBatch(batch, phoneIndex, tempDuplicates, tempNonDuplicates);
    ...
    setUploadMessage('✅ Deduplication check complete!');
    setProgress(100);
    setProcessing(false);
  };
        ...
        <div className="p-6 space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-800">
              <span className="text-blue-600">Dedupe Check</span> Tool
            </h2>
            <p className="text-gray-500 mt-1">Upload your file to find duplicate records</p>
          </div>

          {/* File Upload */}
          <div className="bg-gray-50 rounded-lg p-5 border-2 border-dashed border-gray-200">
            <div className="flex flex-col items-center space-y-4">
              <input type="file" accept=".xlsx, .xls, .csv" onChange={handleFileChange} className="hidden" id="file-upload" />
              <label htmlFor="file-upload" className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md cursor-pointer">
                Choose File
              </label>
            ...
            <div className="mt-4 flex justify-center">
              <button
                ...
                className={`px-5 py-2 rounded-md font-medium text-sm ${
                  processing || !file ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'
                }`}
            ...
            {uploadMessage && (
              <div
                className={`mt-3 text-center text-sm p-2 rounded ${
                  uploadMessage.startsWith('✅') ? 'bg-green-50 text-green-700' : uploadMessage.startsWith('❌') ? 'bg-red-50 text-red-700' : 'bg-blue-50 text-blue-700'
                }`}
              >
                {uploadMessage}
              </div>
